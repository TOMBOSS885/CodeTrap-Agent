...
The Web UI also lets you override provider, API key, model, and API base for a single generation request. Request-level values are used first; `.env` values are only defaults.

Prompt strategy:

- The LLM is asked to generate only the Chinese problem statement metadata.
- It must keep the fixed JSON input/output schema for the selected family.
- It must include illegal-input rules, boundary rules, and at least 8 AI-failure-prone traps.
- The system never trusts the LLM for expected outputs. Expected outputs and reference answers are generated by deterministic local solvers.
